...
- `#SORT:<comma-separated field names, or the sentinel "shuffle">` (§9.2) — the
  ...
  **Local-only for now** — not yet reconciled through Navidrome's comment field
  the way `#TARGETS:` is; see §9.2 for why that's a deliberately separate,
  larger follow-up rather than part of the same change that introduced it.
...
- Remote (Navidrome comment) sync for the `#SORT:` directive `playlist sort`
  introduced (see immediately below) — deliberately deferred as its own
  follow-up rather than folded into `playlist sort` itself. `comment.go`'s
  current encoding is a single tightly-anchored regex-matched suffix,
  `[musicrename:targets=...]`, built around exactly one directive; carrying a
  second one means generalizing that format to multiple independent key/value
  segments (e.g. `[musicrename:targets=...;sort=...]`), which touches
  `parseCommentTargets`/`composeComment` and their reconciliation in
  `pull.go`/`push.go` — all already-shipped and tested — and is realistically
  its own unit of work, comparable in size to the original `#TARGETS:` sync
  retrofit (§9.2, "the `playlists/sums.md5` retrofit," below), not a small
  tack-on. Until this lands, `#SORT:` is local-only: it round-trips correctly
  within one machine's `playlists/` tree, but pushing/pulling a playlist to/from
  Navidrome neither reads nor writes it.
...
experience (and `#SORT:`'s remote sync) remains manual/deferred until the
