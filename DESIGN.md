...
- A directive-order consistency check in `playlist check` — not necessarily
  alphabetical, but the same relative order across every file (`#PLAYLIST:`
  first, since it's the one directive every file has and reads naturally as the
  file's "identity"; the rest in whatever order `WriteGlobalPlaylist` itself
  writes them in, so "correctly ordered" means "matches what our own write
  commands would themselves produce," not a separately-maintained canonical
  order). A misordered file isn't functionally broken — every reader is
...
experience (and the directive-order consistency check) remain until the
