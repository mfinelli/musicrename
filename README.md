...
| `musicbrainz.json.gz`               | Snapshot (generated by `musicbrainz diff`) |
...
`mrr musicbrainz diff` checks whether a MusicBrainz release has changed since
you last looked, so you can catch upstream corrections (a title fix, a changed
artist credit, a corrected composer credit, and similar) even if you've since
made your own edits to the tags yourself.

It's scoped to a single album per run, and refuses to run a library-wide sweep
because MusicBrainz's rate-limiting policy asks API consumers not to poll for
metadata changes, so this is meant to be run occasionally, by hand, against
whichever album you're specifically looking at.

The diff compares MusicBrainz's current data against a local snapshot and not
against the current tag values so any custom edits are not mistaken for upstream
drift.

```sh
mrr musicbrainz diff ~/music/b/beyonce/\[2003\]\ dangerously\ in\ love
